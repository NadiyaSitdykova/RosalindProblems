...
def alignment(s1, s2, scoring_matrix, gap_opening, gap_extension):
    ...
    # compute alignments scores matrices d, w1, w2 and max_score
    max_score = 0
    for i in range(1, n+1):
        for j in range(1, m+1):
            e[i][j] = max(e[i][j - 1] + gap_extension, d[i][j - 1] + gap_opening)
            f[i][j] = max(f[i - 1][j] + gap_extension, d[i - 1][j] + gap_opening)
    ...
    while i > 0 and j > 0 and d[i][j] > 0:
        if d[i][j] == e[i][j]:
            ...
            j -= 1
        if d[i][j] == f[i][j]:
            while i > 0 and (d[i - 1][j] + gap_opening) < (f[i - 1][j] + gap_extension):
                i -= 1
            i -= 1
...
def local_alignment_affine_gap_penalty(raw_lines, s, t):
...
with open("output.txt", 'w') as out:
    out.write("\n".join(map(str, local_alignment_affine_gap_penalty(raw_substitution_matrix, s1, s2))))